...
export default class MysNews extends base {
    ...
    async detalData(data, gid) {
        ...
        if (typeof json == 'object') {
            ...
            data.post.content = data.post.content.replace(/_\([^)]*\)/g, function (t) {
                t = t.replace(/_\(|\)/g, '')
                if (emoticon.has(t)) {
                    return `<img class="emoticon-image" src="${emoticon.get(t)}" alt=""/>`
                } else {
                    return ''
                }
            })
    ...
    async mysNewsTask() {
        ...
        for (let gid of [1, 2, 3, 4, 6, 8]) {
            ...
            if (!lodash.isEmpty(cfg[`${type}announceGroup`])) {
                ...
                if (anno) anno.data.list.forEach(v => {
            ...
            if (!lodash.isEmpty(cfg[`${type}infoGroup`])) {
                ...
                if (info) info.data.list.forEach(v => {
